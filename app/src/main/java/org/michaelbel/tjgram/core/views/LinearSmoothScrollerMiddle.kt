...
open class LinearSmoothScrollerMiddle(context: Context): RecyclerView.SmoothScroller() {
    ...
    private fun calculateTimeForDeceleration(dx: Int): Int = Math.ceil(calculateTimeForScrolling(dx) / .3356).toInt()

    private fun calculateTimeForScrolling(dx: Int): Int = Math.ceil((Math.abs(dx) * millisecondsPerPx).toDouble()).toInt()
