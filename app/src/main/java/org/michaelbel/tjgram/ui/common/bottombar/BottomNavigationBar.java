...
import org.michaelbel.tjgram.ui.common.bottombar.behaviour.BottomNavBarFabBehaviour;
...
public class BottomNavigationBar extends FrameLayout {
    ...
    private OnNavigationItemSelectedListener mTabSelectedListener;
    ...
    public BottomNavigationBar addItems(BottomNavigationItem... items) {
        mBottomNavigationItems.addAll(Arrays.asList(items));
        return this;
    }
     ...
     * @return this, to allow builder pattern
     */
    public BottomNavigationBar setTabSelectedListener(OnNavigationItemSelectedListener tabSelectedListener) {
    ...
    private void sendListenerCall(int oldPosition, int newPosition, boolean forcedSelection) {
        if (mTabSelectedListener != null) {
            //&& oldPosition != -1) {
            if (forcedSelection) {
                mTabSelectedListener.onItemSelected(newPosition);
            } else {
                if (oldPosition == newPosition) {
                    mTabSelectedListener.onItemReselected(newPosition);
                } else {
                    mTabSelectedListener.onItemSelected(newPosition);
                    if (oldPosition != -1) {
                        mTabSelectedListener.onItemUnselected(oldPosition);
                    }
                }
            }
        }
     ...
     */
    public static class SimpleOnTabSelectedListener implements OnNavigationItemSelectedListener {
